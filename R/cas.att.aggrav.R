cas.att.aggrav <- function(net_obj, dt){
  ...
  for(i in 1:(n-1)){
    ...
    vertex[i] <- node_max #cálculo e seleção do vértice a remover + registo do nome do vértice
    strength[i] <- strength_max #registo do valor máximo da força
  ...
  names(df)<-c( "cas.att.aggrav.vertex", "cas.att.aggrav.value", "cas.att.aggrav.number.of.vertices", "cas.att.aggrav.maxcsize", "cas.att.aggrav.cohesion", "cas.att.aggrav.averagepath", "cas.att.aggrav.adhesion", "cas.att.aggrav.edgedensity", "cas.att.aggrav.transitivity", "cas.att.aggrav.radius", "cas.att.aggrav.density", "cas.att.aggrav.centralization", "cas.att.aggrav.components")
  return(df)
}
